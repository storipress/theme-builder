...
export type StateMap<T = any> = T extends object
  ? {
      [K in keyof T]: T[K] extends Function ? Readonly<Ref<T[K]>> : Computed<T[K]>
    }
...
export type ActionMap<T = any> = T extends object
  ? {
      [K in keyof T]: T[K] extends (ctx: ActionContext<any, any>, ...payload: infer P) => infer R
        ? (...payload: P) => R
        : T[K]
    }
...
export type MutationMap<T = any> = T extends object
  ? {
      [K in keyof T]: T[K] extends (state: any, ...payload: infer P) => infer R ? (...payload: P) => R : T[K]
    }
